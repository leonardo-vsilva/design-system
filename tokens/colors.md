...
Definidos como CSS custom properties em `application.tailwind.css`. Referenciados no Tailwind via `bg-primary`, `text-foreground`, etc.
...
> ### Dark mode ⚠️
>
> Ativado pela classe `.dark` no elemento pai (não pelo media query `prefers-color-scheme`). Todos os tokens acima possuem equivalentes no escopo `.dark` em `application.tailwind.css`.